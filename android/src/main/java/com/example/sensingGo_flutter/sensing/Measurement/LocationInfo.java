...
//        locationManager = (LocationManager) mContext.getSystemService(Context.LOCATION_SERVICE);
    ...
    public void startGPS_location(EventChannel.EventSink events) {
        this.eve = events;
        if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED &&
            ...
            HashMap<String, String> gpsLocationMap = new HashMap<String, String>();
            gpsLocationMap.put("Latitude",String.valueOf(userlocationN.getLatitude()));
            gpsLocationMap.put("Longitude",String.valueOf(userlocationN.getLongitude()));
            eve.success(gpsLocationMap);
            locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0, LocationInfo.this, Looper.getMainLooper());
        }
    }
    public void startNetwork_location(EventChannel.EventSink events) {
        this.eve = events;
        if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED &&
                ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            userlocationN = locationManager.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);
            HashMap<String, String> netLocationMap = new HashMap<String, String>();
            netLocationMap.put("Latitude",String.valueOf(userlocationN.getLatitude()));
            netLocationMap.put("Longitude",String.valueOf(userlocationN.getLongitude()));
            eve.success(netLocationMap);
